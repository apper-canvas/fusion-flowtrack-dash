...
const currentStatus = task.status_c || task.status
    onUpdate(task.Id, {
      ...
      completedAt_c: currentStatus === "completed" ? null : new Date().toISOString()
    })
  }
...
<div>Created {format(new Date(task.createdAt_c || task.createdAt), "MMM dd, yyyy")}</div>
          {isCompleted && (task.completedAt_c || task.completedAt) && (
            <div className="text-success-600">
              Completed {format(new Date(task.completedAt_c || task.completedAt), "MMM dd, yyyy")}
            </div>
          )}
        </div>
